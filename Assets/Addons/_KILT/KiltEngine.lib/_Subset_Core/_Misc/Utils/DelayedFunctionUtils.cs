...
namespace Kilt
{
    ...
    public class DelayedFunctionUtils : Singleton<DelayedFunctionUtils>
    {
        ...
        public static void CallFunction(System.Delegate p_functionPointer, float p_time, bool p_tryCreateInstance = true)
        {
            CallFunction(p_functionPointer, null, p_time, p_tryCreateInstance);
        }

        public static void CallFunction(System.Delegate p_functionPointer, object[] p_params, float p_time, bool p_tryCreateInstance = true)
        {
            if (p_tryCreateInstance && Application.isPlaying)
                TryCreateInstance();
            else
                Init();
        ...
        {
            if (Application.isPlaying && DelayedFunctionUtils.GetInstance(true))
                return;
        }
