...
namespace Peppermint.DataBinding.Editor
{
    ...
    public class NodeGUI
    {
        ...
        static NodeGUI()
        {
            ...
            {
                { NodeType.DataContext, "flow node 0 on"},
                { NodeType.DataContextRegister, "flow node 0 on"},
                { NodeType.Binder, "flow node 0 on"},
            };

            unselectedStyleDictionary = new Dictionary<NodeType, string>()
            {
                { NodeType.DataContext, "flow node 0"},
                { NodeType.DataContextRegister, "flow node 0"},
                { NodeType.Binder, "flow node 0"},
            };

            nodeTitleColorDictionary = new Dictionary<NodeType, string>()
            {
                { NodeType.DataContext, ColorUtility.ToHtmlStringRGB(new Color32(38, 156, 255, 255)) },
                { NodeType.DataContextRegister, ColorUtility.ToHtmlStringRGB(new Color32(44, 238, 122, 255)) },
                { NodeType.Binder, ColorUtility.ToHtmlStringRGB(new Color32(255, 200, 0, 255)) },
            };
        }
