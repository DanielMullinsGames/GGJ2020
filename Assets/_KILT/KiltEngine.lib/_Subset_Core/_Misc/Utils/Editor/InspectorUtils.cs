...
namespace KiltEditor
{
    public static class InspectorUtils
    {
        ...
        public static System.IConvertible BitmapPopupInternal(Rect p_rect, bool p_useGuiLayout, string p_label, System.IConvertible p_enum, System.Type p_type)
        {
            ...
            if (EnumExtension.CheckIfIsEnum(p_type, true))
            {
                try
                {
            ...
            else if (EnumExtension.CheckIfIsEnum(p_type, false))
            {
                try
                {
                    if (p_useGuiLayout)
        ...
        static object DrawTypeInternal(Rect p_rect, bool p_useGuiLayout, string p_labelString, object p_object, System.Type p_type)
        {
            ...
            try
            {
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(float)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(long)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(int)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(Color)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(string)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(Vector4)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(Vector3)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(Vector2)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(Rect)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubClassOrImplementInterface(v_type, typeof(bool)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubclass(v_type, typeof(AnimationCurve)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
                ...
                else if (TypeExtension.IsSameOrSubclass(v_type, typeof(Object)))
                {
                    if (p_useGuiLayout)
                    {
                        if (string.IsNullOrEmpty(p_labelString))
        ...
        #region Kilt.Collection Drawers
        ...
        public static int DrawTabs(int p_index, IList<string> p_tabNames, params GUILayoutOption[] p_options)
        {
            return DrawTabs(p_index, p_tabNames, DefaultTabStyle, p_options);
        }

        public static int DrawTabs(int p_index, IList<string> p_tabNames, GUIStyle p_style, params GUILayoutOption[] p_options)
        {
            if (p_tabNames == null)
                p_tabNames = new List<string>();
            EditorGUILayout.BeginHorizontal(p_options);
            GUILayout.Space(4);
            for (int i = 0; i < p_tabNames.Count; i++)
            {
                GUILayout.Space(-4f);
                if (GUILayout.Button(p_tabNames[i] + "", p_style, GUILayout.MaxWidth(EditorGUIUtility.currentViewWidth / p_tabNames.Count)))
                    p_index = i;
                if (p_index == i)
                    EditorGUI.DrawRect(GUILayoutUtility.GetLastRect(), s_selectionColor);
            }
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(-5);
            return Mathf.Clamp(p_index, -1, p_tabNames.Count-1);
        }

        #endregion

        #region Ornament Drawer

        public static void DrawSolidRectangleWithOutline(Rect p_rect, Color p_fillColor, Color p_borderColor, int p_lineWidth = 1)
        {
            if (p_fillColor.a > 0)
                EditorGUI.DrawRect(p_rect, p_fillColor);
            if (p_borderColor.a > 0)
                DrawWireRect(p_rect, p_borderColor, p_lineWidth);
        }

        public static void DrawWireRect(Rect p_rect, Color p_color, int p_lineWidth = 1)
        {
            if (p_color.a > 0)
            {
                EditorGUI.DrawRect(new Rect(p_rect.min, new Vector2(p_lineWidth, p_rect.height)), p_color);
                EditorGUI.DrawRect(new Rect(p_rect.min + new Vector2(0, p_rect.height- p_lineWidth), new Vector2(p_rect.width, p_lineWidth)), p_color);
                EditorGUI.DrawRect(new Rect(p_rect.min + new Vector2(p_rect.width, 0), new Vector2(p_lineWidth, p_rect.height)), p_color);
                EditorGUI.DrawRect(new Rect(p_rect.min, new Vector2(p_rect.width, p_lineWidth)), p_color);
            }
        }

        public static void DrawWireGrid(Rect p_rect, IntVector2 p_amountOfTiles, Color p_borderColor, Color p_gridColor, int p_lineWidth = 1)
        {
            var v_cellSize = new Vector2(p_amountOfTiles.x == 0 ? 0 : p_rect.width / p_amountOfTiles.x, p_amountOfTiles.y == 0 ? 0 : p_rect.height / p_amountOfTiles.y);
            // draw layer border
            DrawWireRect(p_rect, p_borderColor, p_lineWidth);
            // draw tile cells
            if (p_gridColor.a > 0)
            {
                UnityEditor.Handles.color = p_gridColor;
                for (float i = 1; i < p_amountOfTiles.x; i++)
                {
                    EditorGUI.DrawRect(new Rect(p_rect.position + new Vector2(i * v_cellSize.x, 0), new Vector2(p_lineWidth, p_rect.height)), p_gridColor);
                }

                for (float i = 1; i < p_amountOfTiles.y; i++)
                {
                    EditorGUI.DrawRect(new Rect(p_rect.position + new Vector2(0, i * v_cellSize.y), new Vector2(p_rect.width, p_lineWidth)), p_gridColor);
                }
            }
        }
        ...
        static Color s_selectionColor = new Color(0f, 0.3f, 0.8f, 0.4f);
        static GUIStyle s_defaultTabStyle = null;
        static GUIStyle s_defaultTabStyleBold = null;
        static GUIStyle s_defaultTabStyleBig = null;
        static GUIStyle s_defaultTabStyleBigBold = null;

        public static GUIStyle DefaultTabStyle
        {
            get
            {
                if (s_defaultTabStyle == null)
                    FillStyles();
                return s_defaultTabStyle;
            }
        }

        public static GUIStyle DefaultTabStyleBold
        {
            get
            {
                if (s_defaultTabStyleBold == null)
                    FillStyles();
                return s_defaultTabStyleBold;
            }
        }

        public static GUIStyle DefaultTabStyleBig
        {
            get
            {
                if (s_defaultTabStyleBig == null)
                    FillStyles();
                return s_defaultTabStyleBig;
            }
        }

        public static GUIStyle DefaultTabStyleBigBold
        {
            get
            {
                if (s_defaultTabStyleBigBold == null)
                    FillStyles();
                return s_defaultTabStyleBigBold;
            }
        }

        public static Rect CurrentViewRect
        {
            get
            {
                /*try
                {
                    var v_guiViewType = typeof(Editor).Assembly.GetType("UnityEditor.GUIView");
                    var v_property = v_guiViewType.GetProperty("current", BindingFlags.Static | BindingFlags.Public);
                    var v_view = v_property.GetValue(null, null);
                    var v_rect = (Rect)(v_view.GetType().GetProperty("position", BindingFlags.Instance | BindingFlags.Public).GetValue(v_view, null));
                    return v_rect;
                }
                catch { }*/
        ...
        static void FillStyles()
        {
            s_defaultTabStyle = new GUIStyle(EditorStyles.helpBox);
            s_defaultTabStyle.alignment = TextAnchor.MiddleCenter;
            s_defaultTabStyle.clipping = TextClipping.Clip;
            s_defaultTabStyle.wordWrap = true;
            s_defaultTabStyle.fixedHeight = 16;

            s_defaultTabStyleBold = new GUIStyle(s_defaultTabStyle);
            s_defaultTabStyleBold.fontStyle = FontStyle.Bold;

            s_defaultTabStyleBig = new GUIStyle(s_defaultTabStyle);
            s_defaultTabStyleBig.fontSize = 11;
            s_defaultTabStyleBig.fixedHeight = 20;

            s_defaultTabStyleBigBold = new GUIStyle(s_defaultTabStyleBig);
            s_defaultTabStyleBigBold.fontStyle = FontStyle.Bold;
        }

        [InitializeOnLoadMethod]
        static void FillColors()
        {
            s_selectionColor = UnityEditor.EditorGUIUtility.isProSkin ? new Color(0.9f, 0.5f, 0.2f, 0.4f) : new Color(0f, 0.3f, 0.8f, 0.4f);
        }
